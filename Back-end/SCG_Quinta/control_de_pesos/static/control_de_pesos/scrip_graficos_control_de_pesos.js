...
function parseFechaLocal(ts) {
    if (!ts) return null;
    // Recorta microsegundos a milisegundos si vienen 6 dígitos
...
async function cargarProductos(cliente, preselect=''){
    ...
    } catch (e) { console.error('Fallo cargarProductos', e); }
}
F.cliente.addEventListener('change', () => cargarProductos(F.cliente.value, ''));

// ---------- Gráficos ----------
function unidadTiempoDesdeSpan(minDate, maxDate){
    const span = maxDate - minDate;           // en ms
    const H = 60*60*1000, D = 24*H;
    if (span <= 2*H)   return 'minute';       // hasta 2 horas → minutos
    if (span <= 2*D)   return 'hour';         // hasta 2 días  → horas
    if (span <= 90*D)  return 'day';          // hasta ~3 meses → días
    //return 'month';
}
...
function calcularBanda(p){ if(!Number.isFinite(p)) return {min:null,max:null}; const d=p*(TOLERANCIA_PCT/100); return {min:p-d,max:p+d}; }
function sanitizarSerie(data){
    return data.map(d => ({ t: parseFechaLocal(d.ts), y: Number(d.peso_real), receta: Number(d.peso_receta) }))
...
function construirSerieTemporal(regs){
    ...
    if (!pts.length){
    chartLinea = new Chart(ctx,{type:'line',data:{labels:[],datasets:[{label:'Sin datos',data:[]}]},
        options:{responsive:true,maintainAspectRatio:false}});
    F.pesoRef.textContent = 'Peso receta: –'; F.notaTol.textContent = 'Sin datos';
    return;
    }

    const pesoReceta = pts.find(p => Number.isFinite(p.receta))?.receta ?? null;
    const banda = calcularBanda(pesoReceta);
    ...
    const rec  = pesoReceta!=null ? pts.map(p => ({x:p.t,y:pesoReceta})) : [];
    const minL = banda.min!=null  ? pts.map(p => ({x:p.t,y:banda.min}))  : [];
    const maxL = banda.max!=null  ? pts.map(p => ({x:p.t,y:banda.max}))  : [];
    ...
    const yMinFinal = pesoReceta!=null ? Math.min(yMin, pesoReceta*0.9) : Math.floor(yMin - pad);
    const yMaxFinal = pesoReceta!=null ? Math.max(yMax, pesoReceta*1.1) : Math.ceil (yMax + pad);

    chartLinea = new Chart(ctx,{
    type:'line',
    data:{ datasets:[
        { label:'Peso real (gr)', data: real, borderWidth:2, pointRadius:2, tension:.25 },
        ...(rec.length  ? [{ label:'Peso receta (gr)', data: rec,  borderWidth:1, pointRadius:0 }] : []),
        ...(minL.length ? [{ label:'Límite inf',      data: minL, borderWidth:1, pointRadius:0 }] : []),
        ...(maxL.length ? [{ label:'Límite sup',      data: maxL, borderWidth:1, pointRadius:0 }] : []),
    ]},
    options:{
        ...
        x:{
            type:'time',
            time:{
            unit,                    // ← dinámico: 'minute' | 'hour' | 'day' | 'month'
            round: unit,
            displayFormats:{         // formatos legibles por unidad
                minute:'HH:mm',
                hour:'HH:mm',
                day:'MMM d',
                //month:'MMM yyyy',
        ...
        y:{ title:{display:true,text:'gr'}, beginAtZero:false, min:yMinFinal, max:yMaxFinal, grace:0 }
        },
...
function construirHistograma(regs){
    ...
    for (let i=0;i<bins;i++){ const a=min+i*step, b=a+step; labels.push(`${a} a ${b}`); }
    desv.forEach(v => { let idx = Math.floor((v-min)/step); if (idx>=bins) idx=bins-1; if (idx<0) idx=0; counts[idx]++; });
    ...
    options:{
        ...
        scales:{ x:{ title:{display:true,text:'Desviación (gr)'} }, y:{ beginAtZero:true, grace:'8%' } }
    }
    });
}
...
async function cargarDatos(){
    ...
    const ct = r.headers.get('content-type') || '';
    if (!ct.includes('application/json')) { alert('Sesión expirada. Vuelve a iniciar sesión.'); return []; }
    if (!r.ok) return [];
    const j = await r.json();
    return j.ok ? (j.registros || []) : [];
    } catch (e) { console.error('Fallo cargarDatos', e); return []; }
}
...
function descargarExcel(rows){
    ...
    r.id,
    parseFechaLocal(r.ts) || '',                  // Date real -> Excel
    r.cliente ?? '',
    r.producto ?? '',
    r.codigo_producto ?? '',
    ...
    // Formato visible de fecha en columna B (sin tocar el tipo si ya es Date)
...
async function aplicar(){
    ...
    destruirCharts();
    if (data.length){ construirSerieTemporal(data); construirHistograma(data); }
    else {
    const c1 = document.getElementById('chartLinea').getContext('2d');
    chartLinea = new Chart(c1,{type:'line',data:{labels:[],datasets:[{label:'Sin datos',data:[]}]},options:{maintainAspectRatio:false}});
    const c2 = document.getElementById('chartHist').getContext('2d');
    chartHist  = new Chart(c2,{type:'bar', data:{labels:[],datasets:[{label:'Sin datos',data:[]}]},options:{maintainAspectRatio:false}});
    }
...
document.getElementById('btn-limpiar').addEventListener('click', e => {
    e.preventDefault();
    ['f-cliente','f-producto','f-turno','f-lote','f-desde','f-hasta'].forEach(id => { const el=$(id); el.value=''; });
    F.producto.disabled = true; localStorage.removeItem(STORAGE_KEY); history.replaceState(null,'',location.pathname); aplicar();
});
document.getElementById('btn-descargar').addEventListener('click', e => {
    e.preventDefault(); descargarExcel(window.__datosActuales || []);
});
