...
No se crea el archivo `urls.py` ya que este se crea automaticamente al crear la aplicacion, pero no se agrega automaticamente a la aplicacion, por lo que se debe crear el archivo `urls.py` en la carpeta de la aplicacion y agregarlo al archivo `urls.py` de la carpeta `SCG_Quinta`.
Tampoco se crea la carpeta `templates` ni la carpeta `static`, por lo que se deben crear manualmente. La carpeta `templates` debe contener una carpeta con el nombre de la aplicacion, y dentro de esta carpeta se deben crear los archivos html necesarios para el funcionamiento de la aplicacion. La carpeta `static` debe contener una carpeta con el nombre de la aplicacion, y dentro de esta carpeta se deben crear los archivos css y js, la carpeta `images` debe contener las imagenes necesarias para el funcionamiento de la aplicacion. La carpeta `media` debe contener una carpeta con el nombre de la aplicacion, y dentro de esta carpeta se deben crear los archivos necesarios para el funcionamiento de la aplicacion, ambas dentro de `static/nombre_app`.
La carpeta `migrations` debe contener un archivo `__init__.py` para que sea reconocida como una aplicacion de django. Esta carpeta se crea cuando se ejecuta el comando `python manage.py makemigrations`.
El archivo `models.py` debe contener las clases necesarias para el funcionamiento de la aplicacion (tablas de Postgres), estas clases deben heredar de `models.Model`, y deben contener los campos necesarios para el funcionamiento de la aplicacion. Los campos deben ser de tipo `models.CharField`, `models.IntegerField`, `models.FloatField`, `models.DateField`, `models.TimeField`, `models.BooleanField`, `models.EmailField`, `models.URLField`, `models.FileField`, `models.ImageField`, entre otros.
El archivo `views.py` debe contener las vistas necesarias para el funcionamiento de la aplicacion, estas vistas ejecutan los `templates`.
El archivo `urls.py` debe contener las urls necesarias para el funcionamiento de la aplicacion, estas urls deben ser de tipo `path` o `re_path`, y deben contener el nombre de la vista y el nombre de la aplicacion.
##### Superusuario
